...
public class ComCodeTransform extends Transform {
    ...
    private boolean isActivator(CtClass ctClass) {
        //in groovy == is equal when string operate
        try {
            if ("osp.leobert.android.component.AbsComponent" == ctClass.getSuperclass().name)
                return true;

            for (CtClass ctClassInter : ctClass.getInterfaces()) {
                if ("osp.leobert.android.component.lifecycle.ComponentCallbacks" == ctClassInter.name) {
                    return true;
                }
            }
        } catch (Exception e) {
